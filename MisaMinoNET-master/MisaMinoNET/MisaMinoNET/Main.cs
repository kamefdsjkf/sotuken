...
namespace MisaMinoNET {
    ...
    public static class MisaMino {
        ...
        /// <param name="EnemyBest">The amount of incoming garbage which will spawn after the piece is placed.</param>
        public static async void FindMove(int[] queue, int current, int? hold, int height, int[,] field, int combo, int b2b, int garbage, int EnemyBest) {
            if (Interface.alive()) {
                updateQueue(queue);
                ...
                Interface.update_EnemyBest(EnemyBest);
        ...
        public static async void EnemyFindMove(int[] queue, int current, int? hold, int height, int[,] field, int combo, int b2b, int garbage, int[,] enemyfield) {
            if (Interface.alive()) {
                updateQueue(queue);
                Interface.update_current(encodeCurrent(current));
                Interface.update_hold((hold == null)? " " : encodeCurrent(hold.Value));
                Interface.update_field(encodeField(field, height));
                Interface.update_Enemyfield(encodeField(enemyfield, height));
                Interface.update_combo(combo);
                Interface.update_b2b(b2b);
                Interface.update_incoming(garbage);
