...
#define BEG_ADD_Y 1
    MovingSimple AISearch(AI_Param ai_param, const GameField& pool, int hold, Gem cur, int x, int y, const std::vector<Gem>& next, bool canhold, int upcomeAtt, int maxDeep, int & searchDeep) {
        ...
        MovQueue<MovsState> Enemyque(16384);
...

        {
            ...
            for (MovingList::iterator it = movs.begin(); it != movs.end() && !Abort(); ++it) {
                ...
                if (ms.pool_last.RerurnEnemy()) {
                    int beg_y = -5;
                    const int pool_w = pool.width(), pool_h = pool.height();
                    while (pool.row[beg_y] == 0) ++beg_y;
                    for (int x = 0; x < pool_w; ++x) {
                        int cnt = pool_h - beg_y;
                        // フィールド上から確認していく
                        for (int y = beg_y, ey = pool_h + 1; y <= ey; ++y) {
                            if (ms.pool_last.row[y] == ms.pool_last.Enemy_row[y]) {
                                cnt--;
                                if (cnt == 0) {
                                    equals = true;
                                }
                    ...
                    //equals = ms.pool_last.row.size() == ms.pool_last.Enemy_row.size() && std::equal(ms.pool_last.row.cbegin(), ms.pool_last.row.cend(), ms.pool_last.Enemy_row.cbegin());
                ...
                if (equals) {
                    ...
                    Enemyque.push_back();
                    MovingSimple* m;
                    MovQueue<MovsState>* pq = &Enemyque;
                    m = &pq->queue[0].first;
                    return *m;
                }
                que.push_back();
        ...
        }
        {
            ...
            int BestEnemy = pool.EBest;
            int best = abs((m->score - BestEnemy));
            if ( ! GAMEMODE_4W ) {
                for (int i = 1; i < pq_last->size(); i++) {
					MovingSimple* c = &pq_last->queue[i].first;
                    //sprintf_s(num,sizeof(num),"%d\n",m->score);
                    //fputs(num,fp);
                    //sprintf_s(num, sizeof(num), "%d\n", c->score);
                    //fputs(num,fp);
                    //if (best > abs((c->score - BestEnemy))){
                    //    best = abs((c->score - BestEnemy));
                    //    m = c;
                    //}
                    if (m->score > c->score) {
                        m = c;
