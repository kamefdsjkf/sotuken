...
void Bot::updateField(const char* q) {
    ...
    tetris.m_pool.reset(10, rows.size());
        tetris.m_pool.BestEnemy(EnemyBested);
    for (auto &row : rows) {
        tetris.addRow(row);
    }
}

void Bot::updateEnemyField(const char* q) {
    std::string s = q;
    vector<int> rows;
    bool solidGarbage = false;
    int row = 0;
    ...
    tetris.m_pool.Enemyreset(10, rows.size());
    tetris.m_pool.Enemy();
    /*for (auto& row : rows) {
        tetris.EnemyaddRow(row);
    }*/
}
...
void Bot::updateEnemyBest(int EnemyBest) {
    try {
        EnemyBested = EnemyBest;
    }
    catch(...){
        EnemyBested = 0;
    }
